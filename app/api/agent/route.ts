...
// async function generatePlan(prompt: string, existingFiles: any, model?: string): Promise<Plan | null> {
...
//   const rawPlan = await callOllama(planSystemPrompt, model, 0.2); // Slightly higher temp for creativity in planning
...
//   let raw = await callOllama(generateSystemPrompt, model, 0.0);
//   let parsed = extractJSON(raw);

//   // If generation fails, retry once with stricter temperature
//   if (!parsed || !parsed.files) {
//     raw = await callOllama(generateSystemPrompt, model, 0.0);
//     parsed = extractJSON(raw);
//   }
...
//   const rawReview = await callOllama(reviewSystemPrompt, model, 0.3, undefined); // Text format for review, higher temp for insights
...
//     const plan = await generatePlan(prompt, existingFiles, model);
...
//       const rawFallback = await callOllama(fallbackPrompt, model);
...
//     let generated = await generateFilesFromPlan(prompt, plan, existingFiles, model);
...
//     generated = await reviewAndRefineFiles(generated, plan, prompt, model);
